...
namespace vulkan {
  ...
  namespace vertex_buffers {
    VertexBufferHandle create(void* vertices, VkDeviceSize byte_size);
    template <typename VertexT>
    VertexBufferHandle create(const DynamicArray<VertexT>& vertices) {
      return create(vertices._data, sizeof(vertices._data[0]) * vertices._size);
    }

    void update(VertexBufferHandle vertex_buffer, void* vertices, VkDeviceSize byte_size);
    template <typename VertexT>
