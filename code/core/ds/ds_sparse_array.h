...
struct TStaticSparseArray {
  ...
  StaticArray<T, MaxAvailable>     __reverse;
  U32                              _size;
  LType                            __next_id;
};
...
namespace array {
  ...
  init(ArenaHandle arena_handle, LType id, const T& value) {
    TStaticSparseArray<T, LType, MaxInstances, MaxAvailable> sa{
        ...
        .__reverse = init<T, MaxAvailable>(arena_handle),
        ._size     = 0,
        .__next_id = 0,
    };
  }
  ...
  template <typename T, typename LType, U32 MaxInstances, U32 MaxAvailable>
  bool insert(TStaticSparseArray<T, LType, MaxInstances, MaxAvailable>& sa, LType id, T& value) {
  ...
  template <typename T, typename LType, U32 MaxInstances, U32 MaxAvailable>
  void remove(TStaticSparseArray<T, LType, MaxInstances, MaxAvailable>& sa, LType id) {
    if (id >= MaxAvailable || sa.__reverse[id] == -1) return;
    ...
    if (data_index != last_index) {
      ...
      sa.__reverse[data_index]              = sa.__reverse[last_index];
      sa.__lookup[sa.__reverse[data_index]] = data_index;
    }
  ...
  template <typename T, typename LType, U32 MaxInstances, U32 MaxAvailable>
  T* value(TStaticSparseArray<T, LType, MaxInstances, MaxAvailable>& sa, LType id) {
  ...
  LType next_id(TStaticSparseArray<T, LType, MaxInstances, MaxAvailable>& sa) {
    if (sa.count >= MaxInstances) return U32_MAX;
