...
static void SDL_Clay_RenderClayCommands(Clay_SDL3RendererData *rendererData, Clay_RenderCommandArray *rcommands)
{
    for (size_t i = 0; i < rcommands->length; i++) {
        ...
        const SDL_FRect rect = { static_cast<float>(static_cast<int>(bounding_box.x)), static_cast<float>((int)bounding_box.y), static_cast<float>((int)bounding_box.width), static_cast<float>((int)bounding_box.height) };
        ...
        switch (rcmd->commandType) {
            ...
            case CLAY_RENDER_COMMAND_TYPE_BORDER: {
                ...
                if (config->width.left > 0) {
                    ...
                    SDL_FRect line = { rect.x, starting_y, static_cast<float>(config->width.left), length };
                ...
                if (config->width.top > 0) {
                    ...
                    SDL_FRect line = { starting_x, rect.y, length, static_cast<float>(config->width.top) };
                    SDL_RenderFillRect(rendererData->renderer, &line);
                }
